...
阅读及使用方法：在网页端点下一个章节或者到最后，使用系统的返回按钮即可返回上一级的网页，或配合浏览器的返回上一级功能，不同浏览器返回上一级手势不同， 下标文本以Chrome为例，<sub>谷歌浏览器左滑是返回上一级，右滑是下一级</sub> 。对于APP客户端，在下一页之后点下面“返回上一级”就可以回到刚才看的页面了
